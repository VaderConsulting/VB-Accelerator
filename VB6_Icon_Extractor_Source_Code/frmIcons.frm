...
On Error GoTo ErrorHandler
   ...
   If VBGetOpenFileName(sFile, , True, , , , "All Icon Files (*.EXE;*.DLL;*.ICO)|*.EXE;*.DLL;*.ICO|Icon Files (*.ICO)|*.ICO|Executables (*.EXE;*.DLL)|*.EXE;*.DLL|All Files (*.*)|*.*", 1, m_sInFolder, , "ICO", Me.hwnd) Then
      pOpenFile sFile
   End If
      ...
      If (lIndex > 0) Then
         ...
         If (m_iIconCount > 0) Then
            ...
            For iName = 1 To m_iIconCount
               sbrMain.SimpleText = "Loading icon resource " & iName & " of " & m_iIconCount & "..."
...
Dim itmX As ListItem
   ...
   If (m_iIconCount > 0) Then
      ...
      For iIcon = 1 To m_iIconCount
         ...
         Else
            Set itmX = lvwIcons.ListItems.Add(, "C" & iIcon, m_cI(iIcon).ResourceID, "C" & iIcon, "C" & iIcon)
         End If
   ...
   If (m_iIconCount > 0) Then
      ...
      If Len(sFolder) > 0 Then
         ...
         For iItem = 1 To m_iIconCount
            ...
            sbrMain.SimpleText = "Saving icon " & iItem & " of " & m_iIconCount & " to " & sIconFile
            If (FileExists(sIconFile)) Then
               eRes = MsgBox("File " & sIconFile & " already exists, do you want to delete it?", vbYesNoCancel Or vbQuestion)
...
Dim sFile As String
   ...
   If (m_iIconCount > 0) Then
      If Not lvwIcons.SelectedItem Is Nothing Then
         ...
         ' Save the icon:
         Debug.Print
         If (VBGetSaveFileName(sFile, , , "ICO Files (*.ICO)|*.ICO|All Files (*.*)|*.*", , m_sOutFolder, , "ICO", Me.hwnd)) Then
            cI.SaveIcon sFile
            m_cMRU.Add sFile
...
Dim sMsg As String
   If (m_iIconCount > 0) Then
      ...
      End If
   Else
      MsgBox "There are no icon resources to view properties for.", vbInformation
   End If
End Sub
...
On Error Resume Next
   ...
   lH = Me.ScaleHeight - lT + ((sbrMain.Height + 2 * Screen.TwipsPerPixelY) * sbrMain.Visible)
...
Private Sub grdIcons_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
Dim i As Long
...
Private Sub lvwIcons_OLEDragDrop(Data As MSComctlLib.DataObject, Effect As Long, Button As Integer, Shift As Integer, X As Single, Y As Single)
Dim sFile As String
Dim sExt As String
...
Private Sub lvwIcons_OLEDragOver(Data As MSComctlLib.DataObject, Effect As Long, Button As Integer, Shift As Integer, X As Single, Y As Single, State As Integer)
Dim sFile As String
Dim sExt As String
   If Data.Files.Count = 1 Then
      sFile = Data.Files(1)
